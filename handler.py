...
def search_table(attribute_value):
    
    table_name = "paas-table"
    dynamodb = boto3.resource('dynamodb')
    table = dynamodb.Table(table_name)

    response = table.scan(
        FilterExpression=Attr("name").eq(attribute_value)
    )
...
def face_recognition_handler(event,context):
	...
	os.system(f"ffmpeg -i {video_file_path} -r 1 /tmp/{test_image_name}-%3d.jpeg")
	...
	for i in range(1, 100):  # Adjust the range depending on the number of frames extracted
		...
		if os.path.isfile(image_path):
			...
			if True in matches:
				...
				result = search_table(name)
				output_file_name = object_key.split(".")[0]
				output = [output_file_name,result["name"],result["major"],result["year"]]
				print(output)
				...
				s3.upload_file('/tmp/'+output_path, 'paas-output-bucket', output_path)
